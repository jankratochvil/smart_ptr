...
namespace smart_ptr
{
    ...
    template< typename T, size_t N >  size_t find_index(const std::array< T, N >& values, uint32_t value)
    {
    ...
    */
        inline size_t find_index_sse4(const std::array< uint64_t, 8 >& values, uint32_t value)
        {
            __m256i v = _mm256_loadu_si256((const __m256i*)values.data());
    ...
    /*
        inline size_t find_index_sse4(const std::array< uint64_t, 16 >& values, uint32_t value)
        {
            __m256i v = _mm256_loadu_si256((const __m256i*)values.data());
            __m256i vcmp = _mm256_cmpeq_epi64(v, _mm256_set1_epi64x(value));
            unsigned bitmask = _mm256_movemask_ps(_mm256_castsi256_ps(vcmp));
    ...
    */
    ...
    public:
        ...
        {            
            ...
            return find_index(get_local_keys(), 0);
        }
